interface Props {
  onNext: () => void;
}

export default function WelcomeScreen({ onNext }: Props) {
  return (
    <div
      className="flex flex-col min-h-svh relative"
      style={{ background: "rgba(0,0,0,0.9)" }}
    >
      {/* Blurred background content */}
      <div className="flex-1 opacity-20 px-5 pt-14 pointer-events-none">
        <span
          className="text-2xl font-extrabold"
          style={{ color: "#E8A44B", fontFamily: "'Arial Black', sans-serif" }}
        >
          BRAINPAL
        </span>
        <div className="mt-8 flex justify-center">
          <div
            className="rounded-[3rem]"
            style={{
              width: 220,
              height: 380,
              border: "6px solid #2a2a2a",
              background: "#222",
            }}
          />
        </div>
      </div>

      {/* Bottom sheet */}
      <div
        className="absolute bottom-0 left-0 right-0 rounded-t-3xl px-6 pt-3 pb-10"
        style={{ background: "#111" }}
      >
        {/* Drag handle */}
        <div className="flex justify-center mb-4">
          <div className="w-10 h-1 rounded-full" style={{ background: "#444" }} />
        </div>

        {/* Need help */}
        <div className="flex justify-end mb-5">
          <button className="text-sm underline" style={{ color: "#aaa" }}>
            Need help?
          </button>
        </div>

        {/* Title */}
        <h2 className="text-3xl font-bold text-white mb-6 text-center">
          Welcome to BrainPal
        </h2>

        {/* Google button */}
        <button
          onClick={onNext}
          className="w-full py-4 rounded-full flex items-center justify-center gap-3 text-base font-bold mb-4"
          style={{
            background: "linear-gradient(135deg, #F5C97A 0%, #E8A44B 100%)",
            color: "#3d2000",
          }}
        >
          Continue With Google
          <svg width="22" height="22" viewBox="0 0 24 24">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
          </svg>
        </button>

        {/* Privacy policy */}
        <p className="text-center text-xs" style={{ color: "#888" }}>
          By continuing, you have read and agree to our{" "}
          <span className="underline font-medium" style={{ color: "#ccc" }}>Privacy Policy</span>
        </p>

        {/* Home indicator */}
        <div className="flex justify-center mt-8">
          <div className="w-28 h-1 rounded-full bg-white opacity-30" />
        </div>
      </div>
    </div>
  );
}
